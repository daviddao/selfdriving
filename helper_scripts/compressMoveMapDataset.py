...
def main(occup_path, occl_path, occl_path_suffix, road_path, road_path_suffix, lines_path, lines_path_suffix, combined_mask_path, comb_path_suffix, transformation_path, transformation_suffix, out_path, seq_length, step_size, image_size, transformation_only, out_suffix, split_number = 0, split_amount = 1):
    ...
    all_clips = [create_default_element(
        image_size, seq_length, 5) for i in xrange(seq_length)]
    all_transformation = [np.zeros([seq_length, 3, 8], dtype=np.float32) for i in xrange(seq_length)]

    step_offset = int(round(1.0 * step_size / split_amount * split_number))

    start_time = time.time()
    for file_index in xrange(len(occup_list) - seq_length):
        ...
        if file_index >= seq_length - 1 and (file_index - step_offset) % step_size == 0:
            ...
            if file_index == seq_length - 1 or (file_index - step_offset) % 50 == 0:
                if file_index - step_offset > seq_length:
                    CURSOR_UP_ONE = '\x1b[1A'
                    ERASE_LINE = '\x1b[2K'
                    print ""+(CURSOR_UP_ONE + ERASE_LINE) * 4 + CURSOR_UP_ONE
                print "Save " + out_path + "/seq_" + str(file_index).zfill(7)
                pred_calc_time = (len(occup_list) - seq_length - file_index + 1) * \
                    (time.time() - start_time) * 1.0 / (file_index - seq_length + 2)
                print "#" * 100
                if not transformation_only:
                    print "Occup max = "+str(max_occup_diff)+", min = "+str(min_occup_diff)+", mean = "+str(mean_occup_diff / (file_index - seq_length + 2) * step_size)
