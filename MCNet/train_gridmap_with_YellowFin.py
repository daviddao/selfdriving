"""
Train script for gridmaps with YellowFin optimizer (https://github.com/JianGoForIt/YellowFin)
"""
import sys
import time
import imageio
...
sys.path.insert(0,"/lhome/phlippe/scripts/YellowFin/tuner_utils")
from yellowfin import YFOptimizer

def main(lr, batch_size, alpha, beta, image_size, K,
         T, num_iter, gpu, useYellowFin=True):
  ...
  updateG = True
  iters = 0
  prefix  = ("GRIDMAP_MCNET_BIG_LOOP_"
  ...
  with tf.device("/gpu:%d"%gpu[0]):
    ...
    if useYellowFin:
      d_optim = YFOptimizer().minimize(
          model.d_loss, var_list=model.d_vars
      )
      g_optim = YFOptimizer().minimize(
      ...
      while iters < num_iter:
        ...
        for _, batchidx in mini_batches:
          if len(batchidx) == batch_size:
            ...
            output = parallel(delayed(load_gridmap_data)(f, p,img_sze, k, t)
