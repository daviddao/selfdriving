...
class MCNET(object):
    ...
    def __init__(self, image_size, batch_size=32, c_dim=1,  # Input has to be 2 dimensional: First occupancy, last occlusion map
        ...
        self.occlusion_shape = [batch_size, self.image_size[0],
                                self.image_size[1], 1]
        self.ego_motion_shape = [batch_size, 2,3]
    ...
    def build_model(self):
        ...
        pred = self.forward(self.input_tensor, self.motion_map_tensor)
    ...
    def forward(self, input_tensor, motion_maps):
        ...
        for iter_index in range(self.iterations):
            ...
            for t in range(self.K):
                ...
                prediction_output = self.motion_maps_combined(motion_maps[:,:,:,timestep + self.maps_offset,:], decoded_output, reuse=reuse)
    ...
    def motion_enc(self, motion_in, reuse):
        ...
        conv1_1 = relu(conv2d(motion_in, output_dim=self.gf_dim, k_h=3, k_w=3,
                              d_h=1, d_w=1, name='mot_conv1_1', reuse=reuse), useSELU=self.useSELU)
        conv1_2 = relu(conv2d(conv1_1, output_dim=self.gf_dim, k_h=3, k_w=3,
                                         d_h=1, d_w=1, name='mot_conv1_2', reuse=reuse), useSELU=self.useSELU)
        ...
        conv2_1 = relu(dilated_conv2d(h_cell_1, output_dim=self.gf_dim * 2, k_h=3, k_w=3,
                                                 dilation_rate=1, name='mot_dil_conv2_1', reuse=reuse), useSELU=self.useSELU)
        conv2_2 = relu(dilated_conv2d(conv2_1, output_dim=self.gf_dim * 2, k_h=3, k_w=3,
        ...
        conv3_1 = relu(dilated_conv2d(h_cell_2, output_dim=self.gf_dim * 4, k_h=3, k_w=3,
                                                 dilation_rate=1, name='mot_dil_conv3_1', reuse=reuse), useSELU=self.useSELU)
        conv3_2 = relu(dilated_conv2d(conv3_1, output_dim=self.gf_dim * 4, k_h=3, k_w=3,
                                                 dilation_rate=2, name='mot_dil_conv3_2', reuse=reuse), useSELU=self.useSELU)
        conv3_3 = relu(dilated_conv2d(conv3_2, output_dim=self.gf_dim * 4, k_h=3, k_w=3,
                                                 dilation_rate=4, name='mot_dil_conv3_3', reuse=reuse), useSELU=self.useSELU)
    ...
    def dec_cnn(self, h_comb, res_connect, reuse=False):
        ...
        deconv3_3 = relu(deconv2d(relu(tf.add(depool3, res_connect[2])),
                                             ...
                                             d_h=1, d_w=1, name='dec_deconv3_3', reuse=reuse), useSELU=self.useSELU)
        deconv3_2 = relu(deconv2d(deconv3_3, output_shape=shapel3, k_h=3, k_w=3,
                                             d_h=1, d_w=1, name='dec_deconv3_2', reuse=reuse), useSELU=self.useSELU)
        deconv3_1 = relu(deconv2d(deconv3_2, output_shape=shapeout3, k_h=3, k_w=3,
                                             d_h=1, d_w=1, name='dec_deconv3_1', reuse=reuse), useSELU=self.useSELU)
        ...
        deconv2_2 = relu(deconv2d(relu(tf.add(depool2, res_connect[1])),
                                             ...
                                             d_h=1, d_w=1, name='dec_deconv2_2', reuse=reuse), useSELU=self.useSELU)
        deconv2_1 = relu(deconv2d(deconv2_2, output_shape=shapeout3, k_h=3, k_w=3,
                                             d_h=1, d_w=1, name='dec_deconv2_1', reuse=reuse), useSELU=self.useSELU)
    ...
    def motion_maps_combined(self, motion_maps, decoded_output, reuse=False):
        ...
        conv1 = relu(dilated_conv2d(combined_input, output_dim=self.gf_dim, k_h=3, k_w=3,
                                                 dilation_rate=1, name='mot_comb_conv1', reuse=reuse), useSELU=self.useSELU)
        conv2 = tanh(dilated_conv2d(conv1, output_dim=self.c_dim, k_h=3, k_w=3,
                                                 dilation_rate=2, name='mot_comb_conv2', reuse=reuse))
        return conv2
