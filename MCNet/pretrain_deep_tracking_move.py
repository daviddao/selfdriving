...
from move_network import MCNET
from utils import *
...
def main(lr_D, lr_G, batch_size, alpha, beta, image_size, K,
         ...
         useCombinedMask=False, showFutureMaps=False, predOcclValue=1):
    # "/lhome/phlippe/dataset/TwoHourSequence_crop/Train/compressed64x64/"
    data_path = '/lhome/phlippe/dataset/DeepTracking/'
    f = open(data_path + "train_files_"+str(image_size)+"x"+str(image_size)+".txt", "r")
    ...
    prefix = ("GRIDMAP_MCNET_pretrain_s"
                ...
                for _, batchidx in mini_batches:
                    if len(batchidx) == batch_size:
                        ...
                        map_batch = np.zeros((batch_size, image_size, image_size,
                                               sequence_steps * (K + T) + showFutureMaps, 2), dtype="float32")
                        ...
                        output = parallel(delayed(load_gridmap_onmove)(f, img_sze, seq)
                        ...
                        #map_batch = np.concatenate([map_batch] * (sequence_steps * (K + T)), axis=3)
                        ...
                        if np.mod(counter, 200) == 1:
                            ...
                            for seq_step in xrange(sequence_steps * 2):
                                ...
                                maps_road = motion_maps[0, :, :, start_frame:start_frame+1, 1:2].swapaxes(0, 2).swapaxes(1, 2)
                                ...
                                samples_seq_step = np.concatenate(
                                    (maps_lines, samples_seq_step, maps_road, sbatch), axis=0)
                                print("Saving sample ...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--futureMaps", type=str2bool, dest="showFutureMaps",
                        default=False, help="If SELU should be used instead of RELU")
