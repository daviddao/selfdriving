...
def main(lr_D, lr_G, batch_size, alpha, beta, image_size, K,
    ...
    data_path = '/lhome/phlippe/dataset/TwoHourSequence_crop/velHorizon'+str(image_size)+'x'+str(image_size)+'/'
    f = open(data_path + "../train_velnet"+str(image_size)+"x"+str(image_size)+".txt", "r")
                ...
                for _, batchidx in mini_batches:
                    if len(batchidx) == batch_size:
                        ...
                        input_batch = np.zeros((batch_size, image_size, image_size,
                                              sequence_steps * (K + T), 2), dtype="float32")
                        motion_map_batch = np.zeros((batch_size, image_size, image_size,
                                               1, 4), dtype="float32")
                        t0 = time.time()
                        seq_steps = np.repeat(
                        ...
                        output = parallel(delayed(load_gridmap_motion_maps)(f, img_sze, seq)
                                          for f, img_sze, seq in zip(tfiles,
                                                                     shapes,
                                                                     seq_steps))
                        for i in range(batch_size):
                            seq_batch[i] = output[i][0][:,:,1:,:]
                            input_batch[i] = output[i][0][:,:,:-1,:]
                            motion_map_batch[i,:,:,0,:] = output[i][1]
                        motion_map_batch = np.concatenate([motion_map_batch] * (sequence_steps * (K + T)), axis=3)

                        if updateD:
                            _, summary_str = sess.run([d_optim, d_sum],
                                                      feed_dict={model.input_tensor: input_batch,
                                                                 model.motion_map_tensor: motion_map_batch,
                        ...
                        if np.mod(counter, 200) == 1:
                            ...
                            for seq_step in range(sequence_steps * 2):
                                ...
                                motion_orient = motion_maps[0, :, :, start_frame:start_frame+1, 0:3].swapaxes(0, 2).swapaxes(1, 2)
                                motion_intens = motion_maps[0, :, :, start_frame:start_frame+1, 3:4].swapaxes(0, 2).swapaxes(1, 2)
                                samples_seq_step = samples[
                                    0, :, :,start_frame: end_frame].swapaxes(0, 2).swapaxes(1, 2)
                                sbatch = target_occ[
                                    0, :, :,start_frame: end_frame, 0:1].swapaxes(0, 2).swapaxes(1, 2)
                                occ_map_step = occ_map[
                                    0, :, :,start_frame: end_frame].swapaxes(0, 2).swapaxes(1, 2)

                                motion_intens = np.tile(motion_intens, [1,1,1,3])
                                ...
                                samples_seq_step = np.concatenate(
                                    (motion_orient, samples_seq_step, motion_intens, sbatch), axis=0)
                                print("Saving sample ...")
                                save_images(samples_seq_step[:, :, :, :], [2, frame_count + 1],
