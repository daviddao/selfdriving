"""
Functions for creating layers. Used by mcnet.py and similar
Python 2

Taken from https://github.com/rubenvillegas/iclr2017mcnet/blob/master/src/ops.py
...
def conv3d(input_, output_dim,
   ...
   with tf.variable_scope(name, reuse=reuse):
      with tf.device('/cpu:0'):
        ...
        biases = tf.get_variable('biases', [output_dim],
                              initializer=tf.constant_initializer(0.0))
      conv = tf.nn.conv3d(input_, w, strides=[1, d_h, d_w, 1], padding=padding)
