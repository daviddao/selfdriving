...
Every `img-freq` interval the model outputs a text file containing the ground truth and predicted speed and yaw rate, an image containing the ground truth grid map, predicted grid map and intermediate grid maps `train_x_0.png` as well as the ground truth and predicted image channels `train_cam_x_9.png`.
