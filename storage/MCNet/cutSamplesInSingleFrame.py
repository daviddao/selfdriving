"""
CHANGES: input in parser: --col 10 --row 4. now takes 4x10 input, outputs [0:1,1:9] - 1. col is ignored, 3. & 4. row is ignored.
...
def cutSampleInSingleFrames(sample, row_count, column_count, image_base_path):
    ...
    for row in range(2):#range(row_count):
        all_frames_in_row = []
        if row is 0:
            prefile = "_gen_"+step_file+"_"
        else:
            prefile = "_gt_"+step_file+"_"
        for column in range(column_count-1):#range(column_count):
            frame = sample[row * row_step: (row + 1) * row_step, (column + 1) * column_step : (column + 2) * column_step,:]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, dest="sample_file_path",
                            required=True, help="Path to parent directory of samples")
