...
- Adjusted for 3 channel difference
"""
...
class MCNET(object):
  def __init__(self, image_size, batch_size=32, c_dim=1,  # Input has to be 2 dimensional: First occupancy, last occlusion map
    ...
    self.diff_shape = [batch_size, self.image_size[0],
                       self.image_size[1], iterations*(K+T), DIFF_SHAPE]
    self.xt_shape = [batch_size, self.image_size[0], self.image_size[1], iterations, c_dim + 1]
  ...
  def build_model(self):
    ...
    if self.is_train:
      self.target_content, self.occlusion_map = self.extract_loss_target(self.target[:,:,:,:,:])
      # self.occlusion_map = self.create_noisy_occlusion_map(self.occlusion_map)
      ...
      # TODO: For occupancy gridmap BCE loss
    ...
    for iter_index in range(self.iterations):
      ...
      for t in range(self.T):
        if t == 0:
          ...
          res_connect = self.residual(res_m, res_c, reuse=(iter_index > 0))
          x_hat = self.dec_cnn(h_tp1, res_connect, reuse=(iter_index > 0))
        else:
        ...
        if self.c_dim == 3:
          ...
          xt_gray = inverse_transform(xt_input[:,:,:,0:1])

        diff_in_rec = x_hat_gray - xt_gray
        diff_in_rec = tf.concat(axis=3, values=[diff_in_rec, self.pred_occlusion_map])
        #print xt.shape
        #print x_hat.shape
        #print x_hat_gray.shape
        #print xt_gray.shape
        #print diff_in_rec.shape
        xt_input = x_hat
        xt_input = tf.concat(axis=3, values=[xt_input, self.pred_occlusion_map])
  ...
  def load(self, sess, checkpoint_dir, model_name=None):
    ...
    if ckpt is None and False:
      self.saver.restore(sess, '/lhome/phlippe/scripts/MCNet/models/paper_models/KTH/MCNET.model-98502')
      return True
  ...
  def extract_loss_target(self, target):
    ...
    return target_tensor[:,:,:,:,:self.c_dim], (target_tensor[:,:,:,:,self.c_dim:] + 1) / 2
